...
static func _serializable_to_dict(obj : Object) -> Dictionary:
...
static func _variant_to_deep_variant(variant : Variant) -> Variant:
	if variant is Object:
		if variant is Serializeable:
			if variant.resource_path.ends_with(".tres"): #Instantiated
				return _object_to_resource_path_dict(variant)
			return _serializable_to_dict(variant)
		else:
			push_warning("Warning: Object [%s]-%s is not Serializeable." % [variant.get_script().get_global_name(), variant])
			return inst_to_dict(variant)
